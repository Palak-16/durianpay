  ...
  function fetch_details($search, $conn)
  {
    ...
    if ($total_row == 0) {
      foreach ($html->find('div.sp') as $index => $element) {
        ...
        $finalprice = str_replace( array( ',' , '₹' ), '', $price);
        ...
        $sql = "INSERT INTO `products` (`title`, `price` , `url` , `search`,`rating`,`review`,`website`) VALUES ('$title', '$finalprice' , '$url' , '$search','$rating','$review','$website')";
        $result = mysqli_query($conn, $sql);
        if (!$result) {
